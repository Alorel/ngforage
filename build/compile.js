...
gulp.task('compile:esm2015', ['clean:dist:esm2015'], () => {
  return spawn(process.execPath, [ngcPath, '-p', new TsConfigFactory(MODE.DIST_ESM2015).file]);
});

gulp.task('compile:esm5', ['clean:dist:esm5'], () => {
  return spawn(process.execPath, [ngcPath, '-p', new TsConfigFactory(MODE.DIST_ESM5).file]);
});

gulp.task('compile:es5', ['clean:dist:es5'], () => {
  return spawn(process.execPath, [ngcPath, '-p', new TsConfigFactory(MODE.DIST_ES5).file]);
});

gulp.task('compile:demo:aot:prepare', () => {
  return spawn(process.execPath, [ngcPath, '-p', new TsConfigFactory(MODE.DEMO_PRE_AOT).file]);
});

gulp.task('compile:demo:aot:finalise', () => {
  return spawn(
    process.execPath,
    [webpackPath, '--color', '--config', 'webpack.config.js'],
...
gulp.task('compile:demo:watch', () => {
  return spawn(
    process.execPath,
    [webpackPath, '--color', '--config', 'webpack.config.js', '--watch'],
    webpackEnv(MODE.DEMO_JIT)
  );
});
